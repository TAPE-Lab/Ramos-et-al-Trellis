...
#Load datframes
sn_21 <- read.csv (file = '~/Dropbox/From_python_to_R/All_phase_all_levels_for_sankey_21')
sn_21 <- subset(sn_21, (Culture =='PDO' & Replicate == 'C') & (Level != 1) & (Treatment == 'DMSO' | (Treatment == 'S' & Concentration == 3)))
...
nodes <- data.frame(node = c(0:127), name = c('DMSO', 'S', parents))
...
sn_21_all <- sn_21_all %>%mutate(group = case_when(Treatment == 'DMSO' ~ 'group_1', Treatment == 'S' ~ 'group_2'))
sn_21_4 <- sn_21_4 %>%mutate(group = case_when(Treatment == 'DMSO' ~ 'group_1', Treatment == 'S' ~ 'group_2'))

nodes <- nodes %>%mutate(group = case_when(name == ('DMSO') ~ 'group_1', name == ('S') ~ 'group_2', node >= (2) & node <= (22) ~ 'S_phase',
...
my_color <- 'd3.scaleOrdinal().domain(
  ...
  ["grey", "rgba(4, 51, 255, 1)", "rgba(235, 81, 247, 1)", "rgba(234, 82, 246, 1)", "rgba(238, 123, 247, 1)", 
